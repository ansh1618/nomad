...
import { getCmsSection, getSiteSettings } from "@/lib/queries/cms";

export function NomadikCommunity() {
  const { data: section } = useQuery({
    queryKey: ["cms", "community_cta"],
    queryFn: () => getCmsSection("community_cta"),
    staleTime: 1000,
  });
  ...
  const title = section?.title || "The Nomadik Explorer Community";
  const subtitle = section?.subtitle || "Join the Nomadik Family";
  const content = section?.content as any;
  const description = content?.description || "Join 5,000+ explorers in our private WhatsApp community. Get early access to departures, connect with co-travelers, share trip photos, and hear real road stories before anyone else.";
  const ctaLabel = content?.cta_label || "Join WhatsApp Community";
  
  // WhatsApp link fallback logic: settings or BRAND config
        ...
        <Reveal delay={3}>
          <div className="flex flex-wrap justify-center gap-8 pt-4">
            <div className="flex items-center gap-2 text-white/80">
              ...
              <span className="text-sm font-sans">15,000+ Members</span>
            </div>
            <div className="flex items-center gap-2 text-white/80">
