...
import { ShieldCheck, CreditCard, Loader2, Lock, AlertTriangle, CheckCircle2, Building, Calendar, Users, Hotel, Tag } from "lucide-react";
...
export function PaymentStep({ data, updateData, onNext, onPrev, journey, isSidebar = false, pricing }: any) {
  ...
  const hasValidPrice = Boolean(pricing && pricing.total > 0);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!loaded) throw new Error("Payment gateway connection failed. Please check internet connection.");
          ...
          handler: async (response: any) => {
            try {
              ...
              if (!verification.success) {
                reject(new Error("Payment confirmation pending. Contact support if debited."));
                return;
              }
  ...
  const departureDateStr = data.departureDate
    ? new Date(data.departureDate).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })
    : null;
  ...
  return (
    <div className="w-full max-w-xl mx-auto space-y-6 animate-fade-in py-2 px-1 box-border">
      ...
      <div className="text-center space-y-1">
        <h2 className="text-2xl sm:text-3xl font-display font-bold text-primary">Secure Payment</h2>
        <p className="text-xs sm:text-sm text-muted-foreground font-poppins">
          Complete your booking securely using Razorpay.
        </p>
      </div>

      {/* ── WARNING BOX (If departure is not selected) ── */}
      {!hasDeparture && (
        <div className="bg-amber-50/90 border border-amber-200/90 p-4 rounded-[16px] text-amber-900 flex items-start gap-3 shadow-sm">
          ...
          <div className="text-xs font-poppins space-y-0.5">
            <p className="font-semibold text-amber-950">Select your preferred departure to continue securely.</p>
            <p className="text-amber-800/80">Please step back and choose an available departure batch date.</p>
          </div>
        </div>
      )}
      ...
      <div className="w-full bg-white border border-border/80 p-6 sm:p-7 rounded-[20px] shadow-soft space-y-6 box-border">
        ...
        {/* Total Amount Top Banner */}
        <div className="bg-muted/30 border border-border/60 p-5 rounded-[16px] flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
          ...
          <div className="sm:text-right">
            <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-700 border border-emerald-200/70 text-[10px] font-poppins font-bold px-2.5 py-1 rounded-full uppercase tracking-wider">
              <CheckCircle2 className="h-3 w-3" /> GST Included (5%)
            </span>
            {pricing?.gst > 0 && (
              <p className="text-[11px] text-muted-foreground font-poppins mt-1">
                GST Amount: ₹{pricing.gst.toLocaleString("en-IN")}
              </p>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3.5 pt-1 border-t border-border/50 text-xs font-poppins">
          <div className="flex justify-between items-center gap-4">
            ...
            <span className="font-semibold text-primary text-right line-clamp-1">{journey?.name || "Selected Journey"}</span>
          </div>
          ...
          <div className="flex justify-between items-center gap-4">
            ...
            <span className="font-semibold text-foreground text-right">
              {departureDateStr || "Selected Batch"}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center gap-4">
            ...
            <span className="font-semibold text-foreground text-right">
              {data.selectedRoomObj?.type || data.selectedRoomObj?.title || "Standard Room Sharing"}
            </span>
          </div>
          ...
          {data.coupon && (
            <div className="flex justify-between items-center gap-4 text-green-700">
              <span className="text-[11px] uppercase font-semibold tracking-wider flex items-center gap-1.5">
                <Tag className="h-3.5 w-3.5 text-green-600 shrink-0" /> Coupon ({data.coupon.code})
              </span>
              <span className="font-bold text-right">- ₹{(pricing?.couponDiscount ?? 0).toLocaleString("en-IN")}</span>
            </div>
          )}
          ...
          {pricing?.addonsTotal > 0 && (
            <div className="flex justify-between items-center gap-4">
              ...
              <span className="font-semibold text-foreground text-right">+ ₹{pricing.addonsTotal.toLocaleString("en-IN")}</span>
            </div>
          )}

          <div className="pt-3 border-t border-dashed border-border flex justify-between items-center gap-4">
            <span className="text-[11px] uppercase font-bold text-muted-foreground tracking-wider">
              Subtotal
            </span>
            <span className="font-semibold text-foreground text-right">₹{(pricing?.subtotal ?? 0).toLocaleString("en-IN")}</span>
          </div>
          ...
          <div className="flex justify-between items-center gap-4">
            ...
            <span className="font-semibold text-foreground text-right">₹{(pricing?.gst ?? 0).toLocaleString("en-IN")}</span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 py-2.5 px-3 bg-muted/20 border border-border/60 rounded-[14px] text-[11px] text-muted-foreground font-poppins font-medium text-center">
          <span className="flex items-center gap-1"><Lock className="h-3 w-3 text-emerald-600 shrink-0" /> 256-bit Encryption</span>
          <span className="text-border">•</span>
          <span className="flex items-center gap-1"><ShieldCheck className="h-3 w-3 text-emerald-600 shrink-0" /> PCI DSS Compliant</span>
          <span className="text-border">•</span>
          ...
          >
            {isProcessing ? (
              ...
              <>
                ...
                <span>Pay ₹{(pricing?.total ?? 0).toLocaleString("en-IN")} Securely</span>
              </>
            )}
          </Button>
      ...
      <div className="text-center space-y-1 text-xs text-muted-foreground/80 font-poppins pt-1">
        <p className="flex items-center justify-center gap-1 font-medium">
          <Lock className="h-3.5 w-3.5 text-emerald-600 shrink-0" /> Payments are securely processed by Razorpay.
        </p>
