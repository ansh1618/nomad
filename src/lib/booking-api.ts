 ...
 *
 * Rules:
 *  - Never expose CASHFREE_SECRET_KEY in here (that's cashfree-fns.ts / webhook.ts)
...
export async function lockDepartureSeats(
  departureId: string,
  count: number
): Promise<void> {
...
export async function validateCoupon(
  ...
  const minAmount = (coupon as any).min_amount ?? 0
...
export async function confirmBookingAfterPayment(
  ...
  await adminClient.from('booking_timeline').insert({
    ...
    description: `Payment of ₹${amountPaid} confirmed via Cashfree`,
    actor: 'SYSTEM',
    metadata: { cashfree_payment_id: cashfreePaymentId, order_id: cashfreeOrderId },
  })
  ...
  await adminClient.from('notifications').insert({
    ...
    message: `Booking confirmed. Amount: ₹${amountPaid.toLocaleString('en-IN')}`,
    type: 'SUCCESS',
